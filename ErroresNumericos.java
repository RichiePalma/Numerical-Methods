...
//Ricardo Palma Mendoza A01226922
public class ErroresNumericos{
	public double errorAbsoluto(double trueVal, double aproxVal){		//Err(abs) = | Valor(verdader) - Valor (Aproximado) |
		return (double)Math.round(Math.abs(trueVal - aproxVal)*1000d)/1000d;   //Round value up to 3 decimals
	}
	
	public double errorRelativo(double trueVal, double aproxVal){	//Err(rel) = | (Val(true) - Val(aprox)) / Val(true) | 
		double errAbs = errorAbsoluto(trueVal,aproxVal);
		return (double)Math.round(Math.abs(errAbs / trueVal)*1000d)/1000d ;
	}
	
	public double errorRelativoNormalizado(double aproxActual, double aproxAnterior){  //Err(Norm) = | (Val(actual) - Val(ant)) / Val(actual)|
		return errorRelativo(aproxActual,aproxAnterior);
	}
	
	public double aproxEuler(long n){
		return Math.pow((1 + (1d /n)), n); // Could give Underflow 
		
	}
	
	public double aproxEulerTol(int n) { 
